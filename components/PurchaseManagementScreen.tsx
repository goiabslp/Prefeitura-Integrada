  ...
  const handleBudgetUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && budgetUploadOrder) {
      ...
      reader.onloadend = () => {
        ...
        onUpdatePurchaseStatus?.(budgetUploadOrder.id, 'aprovacao_orcamento', 'Documento de orçamento anexado para análise.', base64);
      ...
      {budgetUploadOrder && createPortal(
        <div className="fixed inset-0 z-[600] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-fade-in">
           <div className="w-full max-w-lg bg-white rounded-[2.5rem] shadow-2xl border border-white/20 overflow-hidden flex flex-col animate-slide-up">
              ...
              <div className="p-8">
                 <div onClick={() => fileInputRef.current?.click()} className="w-full aspect-[16/9] border-2 border-dashed border-slate-200 rounded-[2rem] flex flex-col items-center justify-center gap-4 hover:border-purple-500 hover:bg-purple-50/50 transition-all cursor-pointer group">
                    <input type="file" ref={fileInputRef} className="hidden" accept="image/*,application/pdf" onChange={handleBudgetUpload} />
              ...
              <div className="p-6 bg-slate-50 border-t border-slate-100 flex justify-end"><button onClick={() => setBudgetUploadOrder(null)} className="px-8 py-3 bg-white text-slate-500 font-black text-xs uppercase tracking-[0.2em] rounded-2xl border border-slate-200 hover:bg-slate-100 transition-all">Voltar</button></div>
           </div>
        </div>,
        document.body
      )}
